...
def run_codetypo_stdin(
    ...
    """
    Run the codetypo CLI with the given stdin text and return the number of lines produced on stdout.
    
    Parameters:
        text (str): Text provided to codetypo via standard input.
        args (Tuple[Any, ...]): Additional command-line arguments to pass to codetypo; the stdin file indicator ('-') is appended automatically.
        cwd (Optional[Path]): Working directory for the codetypo subprocess; if None, the current working directory is used.
    
    Returns:
        int: Number of lines in codetypo's stdout (count of newline characters).
    """
    proc = subprocess.run(  # noqa: S603
...
def test_args_from_file(
    ...
    """
    Test that codetypo accepts an argument-file (@file) containing multiple paths and reports expected spelling suggestions.
    
    Creates three temporary files and an argument-list file referencing them, runs the codetypo CLI with the @file syntax, asserts that each target file produces the expected suggestion lines in stdout, and invokes cs_.main with the expanded arguments to exercise the direct-call code path for coverage.
    """
    import textwrap
