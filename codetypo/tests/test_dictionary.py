...
def _check_err_rep(
    ...
    """
    Validate a dictionary error token and its correction string for formatting rules and optional aspell membership.
    
    Parameters:
        err (str): The error token (must contain no whitespace or commas).
        rep (str): The correction string; one or more corrections separated by commas. When multiple corrections are present the string must end with a trailing comma.
        in_aspell (tuple[Optional[bool], Optional[bool]]): Two optional flags indicating expected aspell membership: first for `err`, second for each correction. Use `None` to skip aspell checks for that side.
        fname (str): Source filename used in assertion messages.
        languages (tuple[Iterable[str], Iterable[str]]): Two iterables of language identifiers passed to aspell checks: first for `err`, second for corrections.
    
    Raises:
        AssertionError: If any formatting rule is violated or an aspell membership expectation is not met. The message explains the specific failure.
    """
...
def test_dictionary_looping(
    ...
    """
    Validate a dictionary file for formatting, uniqueness, and cross-dictionary conflicts.
    
    Performs these checks:
    - Parses the file at `fname` into error keys and their corrections.
    - Ensures each error key is unique within the file.
    - Ensures each error matches the default word regex (`word_regex_def`).
    - Ensures no correction in a file is itself listed as an error in the same file.
    - For each previously-seen dictionary, ensures no error duplicates appear across files unless the associated correction lists are identical.
    - Ensures no correction in one dictionary is an error in another, and vice versa, except for pairs listed in `allowed_dups`.
    - Records checked pairs in the global `global_pairs` set and stores the parsed mapping in `global_err_dicts`.
    
    Parameters:
        fname (str): Path to the dictionary file to validate.
        in_aspell (tuple[bool, bool]): Tuple of flags indicating aspell membership for the error and correction sides (used for test parametrization).
        in_dictionary (tuple[bool, bool]): Tuple indicating membership in built-in dictionaries for the error and correction sides (used for test parametrization).
    """
    this_err_dict = {}
