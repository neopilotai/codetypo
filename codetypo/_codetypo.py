...
_builtin_dictionaries = (
    ...
    (
        "en-GB_to_en-US",
        "intermediate file for generating en_to_en-OX dictionary",
        "_en-GB_to_en-US",
        None,
        None,
        None,
...
class FileOpener:
    ...
    def init_chardet(self) -> None:
        """
        Initialize the character-encoding detector using the chardet library.
        
        Sets self.encdetector to a new chardet UniversalDetector instance.
        
        Raises:
            ImportError: if the chardet package is not available or cannot be imported.
        """
        try:
...
def parse_options(
    ...
    """
    Parse command-line arguments and configuration files for the program.
    
    Parameters:
        args (Sequence[str]): Sequence of command-line arguments (typically sys.argv[1:]).
    
    Returns:
        Tuple[argparse.Namespace, argparse.ArgumentParser, List[str]]: 
            - argparse.Namespace: parsed command-line and merged configuration options.
            - argparse.ArgumentParser: the parser used (configured with application-specific flags).
            - List[str]: list of configuration files that provided options (e.g., 'setup.cfg', '.codetyporc', or a supplied config path).
    """
...
def ask_for_word_fix(
    ...
    """
    Prompt the user (when interactivity is enabled) to confirm or choose a correction for a matched misspelled word in a line.
    
    Parameters:
        line (str): The full text line containing the match.
        match (re.Match[str]): The regex match object locating the misspelled word within `line`.
        misspelling (Misspelling): Misspelling record providing candidate fix data; this object may be mutated (its `fix` and `data` attributes can be updated based on user input).
        interactivity (int): Bitflags controlling interactive behavior:
            - bit 0 (value 1): prompt for simple confirmation "Y/n" when a single fix is available.
            - bit 1 (value 2): present a numbered list of alternatives when multiple fixes are provided.
            A non-positive value disables prompting.
        colors (TermColors): Terminal color controls used to highlight the matched word when displaying prompts.
    
    Returns:
        tuple:
            - fix_applied (bool): `True` if a correction should be applied, `False` otherwise.
            - fixed_word (str): The chosen replacement word (case-adjusted to match the original word's casing).
    """
...
def parse_file(
    ...
    """
    Scan a single file or stdin for misspelled words, optionally report or apply fixes, and update summary counts.
    
    Processes the given filename (or "-" for stdin), iterates over each line, applies ignore rules (inline, global, URI-based, and case-sensitive ignores), detects misspellings using the provided misspellings map, and either prints diagnostics, prompts interactively for fixes, or applies replacements to the file contents depending on options. When changes are applied, the file is rewritten using the encoding returned by file_opener.
    
    Parameters:
        filename: Path to the file to scan, or "-" to read from stdin.
        colors: Terminal color helper used to colorize printed output.
        summary: Optional Summary object to update aggregated counts for corrected words.
        misspellings: Mapping from lowercased misspelled words to Misspelling objects (contains fix and metadata).
        ignore_words_cased: Set of case-sensitive words to skip.
        exclude_lines: Set of exact line texts to skip during scanning.
        file_opener: FileOpener used to open files and determine encoding.
        word_regex: Compiled regex used to extract candidate words from lines.
        ignore_word_regex: Optional compiled regex used to mask ignore-word patterns inside text before extraction.
        uri_regex: Compiled regex that matches URIs/emails for URI-aware ignore handling.
        uri_ignore_words: Set of words which, when present inside matched URIs, should be ignored; if it contains "*" all URIs are removed before word extraction.
        context: Optional (before, after) tuple controlling how many surrounding lines to print when showing context.
        options: Parsed command-line options controlling behavior (interactive mode, write-changes, quiet levels, filename checks, stdin formatting, etc.).
    
    Returns:
        bad_count (int): Number of uncorrected misspellings reported or detected during this file's processing.
    """
    bad_count = 0
    lines = None
    changed = False
